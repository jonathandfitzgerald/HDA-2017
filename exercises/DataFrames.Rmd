...
```{r}
setwd("~/Documents/Northeastern/HDA-2017")
```
...
censusPop <- read.csv(file="./data/1840-census-data.csv")[ , c("QualifyingAreaName", "TotalPopulation", "Male", "Female")]
censusPop %>% arrange(desc(TotalPopulation)) %>% head
censusPopLarge = censusPop %>% filter(TotalPopulation > 75000)
...
slaveCount = census[c("county", "FreeColoredPopulation", "SlavePopulation")]

slaveCount = slaveCount %>% mutate("ratio" = round(FreeColoredPopulation/SlavePopulation, digits=2)) %>% arrange(-ratio)
...
census_pub <- select(census, QualifyingAreaName, Newspapers, Newspapers_Daily, Newspapers_Weekly, Newspapers_SemiTriWeekly, Periodicals)
census_pub <- separate(census_pub, QualifyingAreaName, into = c("county", "state"), sep = "\\, ")
census_pub$county = NULL
census_pub[is.na(census_pub)] <- 0
census_pub_long <- gather(census_pub,"publication_type", "count", -state)
census_pub_long <- group_by(census_pub_long,state) %>% summarise(total_publications=sum(count))


#Which state has the most literate whites (over 20)

census_all <- read.csv(file = "data/1840-census-data.csv")
census_all <- census_all[ , c("QualifyingAreaName", "TotalPopulation", "LiterateWhiteAge20andOver")]
census_all <- separate(census_all, QualifyingAreaName, into = c("county", "state"), sep = "\\, ")
census_pop <- census_all %>% group_by(state) %>% summarise(total_pop=sum(TotalPopulation))
census_lit <- census_all %>% group_by(state) %>% summarise(total_literate=sum(LiterateWhiteAge20andOver))
census_litpop <- inner_join(census_pop,census_lit)
census_litpop <- arrange(census_litpop, -total_literate)


#Which state has the most literate whites per capita
census_litpop <- census_litpop %>% mutate("ratio" = round(total_literate/total_pop, digits=2)*100) %>% arrange(-ratio)
