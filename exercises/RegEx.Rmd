...
crewlist$year = crewlist$ApproximateDeparture %>% as.character() %>% gsub(".*([0-9]{4}).*","\\1",.)

#Could also do: ((\d+)\/) to match the month and day, leaving just the year. But the above keeps the year, and that's easier.
...
crewlist$first = crewlist$FullName %>% as.character() %>% gsub("^([A-Za-z]*),","\\2",.)
crewlist$last = crewlist$FullName %>% as.character() %>% gsub(", (.*)","",.)
...
narrative = scan("../data/NarrativeWilliamWellsBrown.txt",sep="\n",what="raw",skip = 365)

narrative = paste(unlist(narrative), collapse =" ")

quotes = narrative %>% gsub('(.*?)(\".*?\")','\\2',.)
...
words = c("deceive","conceive","ceiling","weigh","neigh","tied","lied")

words2 = words %>% gsub('(?<!c)(e)(i)','\\2\\1',.,perl = TRUE)
